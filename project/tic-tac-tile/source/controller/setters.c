...
#define MODE_TO_PALETTE(mode) BG_PALETTE_SUB[((mode) == SINGLE_PLAYER ? 12 : ((mode) == TWO_PLAYER_LOCAL ? 5 : 8))]
#define SPEED_TO_PALETTE(speed) BG_PALETTE_SUB[((speed) == SLOW ? 7 : ((speed) == MEDIUM ? 11 : 1))]
#define SELECTED_COLOR RGB15(5, 0, 30)

#define SPEED_COLOR(speed) ((speed) == SLOW ? RGB15(7, 26, 14) : ((speed) == MEDIUM ? RGB15(28, 14, 5) : RGB15(21, 6, 6)))
#define PROGRESS_COLOR(mode, speed) ((mode) == T_UNUSED ? RGB15(31, 31, 31) : SPEED_COLOR(speed))
...
void set_progress_time_left(u8 new_time_left) {
    ...
    progress_time_left = new_time_left;
}
